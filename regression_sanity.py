"""Sanity check: point-estimate regression x_hat = f(y) on the amplitude-only,
multi-source problem.

Trains a direct regressor (same MMDiT backbone as train.py) to predict each
source's unit-cube amplitude A from the WDM conditioning y, isolating "is y
digestible" from "does flow matching converge". Uses lisa.get_train_batch (which
now returns the full 8-param geodesic), so the 7 other params (f0, fdot, sky,
orientation) vary as random nuisances per source instead of being pinned -- the
harder version of the earlier fixed-nuisance A-only test.

Knobs (env): N_SOURCES (default 2), NOISE_SCALE (default 1.0, read inside lisa).
...
def main(seconds: float):
    ...
    x_dim = 1  # regress A only; the other 7 lisa params vary as nuisances
    ...
    x1_eval = np.asarray(xe + (1.0 - te)[:, None, None] * dxe)[..., A_IDX:A_IDX + 1]
    ...
    def checkpoint_and_eval(net, losses, elapsed):
        ...
        for i, (ax, name) in enumerate(zip(axes[0], PARAM_NAMES)):
            ax.scatter(x1_eval[:, i, 0], pred[:, i, 0], s=3, alpha=0.3)
            ax.plot([0, 1], [0, 1], "k--", lw=1)
            ax.set_xlabel(f"true {name}"); ax.set_ylabel(f"pred {name}"); ax.set_title(name)
        fig.suptitle(f"A|y reconstruction ({TAG}, {elapsed/3600:.1f}h)")
    ...
    while time.monotonic() - t0 < seconds:
        ...
        x1 = (xt + (1.0 - t)[:, None, None] * dx)[..., A_IDX:A_IDX + 1]
