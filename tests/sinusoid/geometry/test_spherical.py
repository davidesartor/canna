...
from canna.sinusoid.geometries import Spherical


def sphere_point(lon, lat, radius):
...
def test_geometry_tangent_dim_matches_the_ambient_dim():
    geo = Spherical()
    x0 = sphere_point(0.3, 0.4, 2.0)
    x1 = sphere_point(1.1, -0.2, 2.0)
    tangent = geo.log_map(x0, x1)
    assert tangent.shape[-1] == x0.shape[-1]


def test_a_lifted_angle_pair_lies_on_the_sphere_of_the_right_radius():
    x = sphere_point(0.3, 1.1, 2.0)
    assert jnp.allclose(jnp.linalg.norm(x), 2.0, atol=1e-4)
    assert jnp.allclose(jnp.arctan2(x[1], x[0]), 0.3, atol=1e-4)
    assert jnp.allclose(jnp.arctan2(x[2], jnp.hypot(x[0], x[1])), 1.1, atol=1e-4)
