...
def test_geometry_property_is_referentially_stable():
    problem = LisaGB(n_sources=2)
    x0 = problem.sample_point(jr.key(5), window(problem))
    x1 = problem.sample_point(jr.key(6), window(problem))
...
def test_geometry_log_exp_roundtrip_matches_up_to_source_permutation():
    ...
    problem = LisaGB(n_sources=3)
    x0 = problem.sample_point(jr.key(14), window(problem))
    x1 = problem.sample_point(jr.key(15), window(problem))
...
def test_geometry_log_map_zero_at_identity():
    problem = LisaGB(n_sources=2)
    x0 = problem.sample_point(jr.key(9), window(problem))
...
def test_problem_log_map_matches_its_geometry():
    problem = LisaGB(n_sources=2)
    x0 = problem.sample_point(jr.key(11), window(problem))
    x1 = problem.sample_point(jr.key(12), window(problem))
...
def test_sky_block_is_a_unit_vector():
    problem = LisaGB(n_sources=2)
    sky = problem.sample_point(jr.key(0), window(problem))[:, 3:6]
    assert jnp.allclose(jnp.linalg.norm(sky, axis=-1), 1.0)


def test_orientation_block_is_a_unit_vector():
    problem = LisaGB(n_sources=2)
    orientation = problem.sample_point(jr.key(0), window(problem))[:, 6:9]
...
def test_geodesic_starts_at_the_base_point():
    problem = LisaGB(n_sources=2)
    x0, x1 = problem.sample_point(jr.key(3), window(problem)), problem.sample_point(
        jr.key(4), window(problem)
    )
    assert jnp.allclose(problem.geodesic(jnp.array(0.0), x0, x1), x0)
...
def test_sample_point_matches_physical_to_flow_of_sample_physical_same_key():
    # sample_point's body is literally `physical_to_flow(sample_physical(key, f), f)`,
    # so given the same key it draws the same source set as sample_physical.
    problem = LisaGB(n_sources=2)
    key = jr.key(13)
    p = problem.sample_physical(key, window(problem))
    x = problem.sample_point(key, window(problem))
    assert jnp.allclose(
