"""LisaFlow: shapes, permutation invariance over binaries, and the trailing window f."""
...
from canna.lisa import LisaFlow

S, X, H, W, C, HIDDEN, HEADS = 4, 11, 8, 8, 3, 16, 2
F = 3.0e-3
...
def test_output_triple_shapes():
    args = (rand((S, X), 1), jnp.asarray(0.3), rand((H, W, C), 2), jnp.asarray(F))
    dx, xt, yt = flow()(*args)
...
def test_takes_one_example_and_vmaps():
    dx, xt, yt = jax.vmap(flow())(
        rand((3, S, X), 1), rand((3,), 3), rand((3, H, W, C), 2), rand((3,), 4) + F
    )
...
def test_batch_rows_are_independent():
    net = perturbed(flow())
    x, t = rand((3, S, X), 1), rand((3,), 3)
    y, f = rand((3, H, W, C), 2), rand((3,), 4) + F
    dx = jax.vmap(net)(x, t, y, f)[0]
    close(dx, jnp.stack([net(x[i], t[i], y[i], f[i])[0] for i in range(3)]))


@pytest.mark.parametrize("sources", [1, 2, 7])
def test_source_count_is_free(sources):
    dx, xt, _ = flow()(
        rand((sources, X), 1), jnp.asarray(0.3), rand((H, W, C), 2), jnp.asarray(F)
    )
...
def test_velocity_and_x_target_are_distinct_heads():
    dx, xt, _ = flow()(
        rand((S, X), 1), jnp.asarray(0.3), rand((H, W, C), 2), jnp.asarray(F)
    )
    differs(dx, xt)
...
def test_finite_at_the_time_endpoints(t):
    outs = flow()(rand((S, X), 1), jnp.asarray(t), rand((H, W, C), 2), jnp.asarray(F))
    assert all(bool(jnp.all(jnp.isfinite(o))) for o in outs)


def test_ignores_conditioning_at_init():
    net, x = flow(), rand((S, X), 1)
    a = net(x, jnp.asarray(0.1), rand((H, W, C), 2), jnp.asarray(F))[0]
    b = net(x, jnp.asarray(0.9), 10.0 * rand((H, W, C), 9), jnp.asarray(9.0e-3))[0]
    close(a, b, atol=0.0)
...
def test_velocity_depends_on_t_once_perturbed():
    ...
    f = jnp.asarray(F)
    differs(net(x, jnp.asarray(0.1), y, f)[0], net(x, jnp.asarray(0.9), y, f)[0])


def test_velocity_depends_on_y_once_perturbed():
    net, x, t = perturbed(flow()), rand((S, X), 1), jnp.asarray(0.3)
    f = jnp.asarray(F)
    differs(net(x, t, rand((H, W, C), 2), f)[0], net(x, t, rand((H, W, C), 12), f)[0])


def test_velocity_depends_on_f_once_perturbed():
    """f is the window's centre frequency: the flow must know which band it is looking at"""
    net, x, t, y = (
        perturbed(flow()),
        rand((S, X), 1),
        jnp.asarray(0.3),
    ...
    differs(net(x, t, y, jnp.asarray(1.0e-3))[0], net(x, t, y, jnp.asarray(9.0e-3))[0])


def test_f_and_t_enter_through_the_same_conditioning_vector():
    # c = t_mlp(t_embed(t)) + f_mlp(f_embed(f)), so f cannot reach the output by any path
    # that bypasses the modulation gates -- at init it is as inert as t
    net, x, y = flow(), rand((S, X), 1), rand((H, W, C), 2)
    t = jnp.asarray(0.3)
    close(
        net(x, t, y, jnp.asarray(1.0e-3))[0],
        net(x, t, y, jnp.asarray(9.0e-3))[0],
        atol=0.0,
    )
...
def test_y_target_depends_on_x_once_perturbed():
    ...
    f = jnp.asarray(F)
    differs(net(rand((S, X), 1), t, y, f)[2], net(rand((S, X), 8), t, y, f)[2])


def test_permutation_equivariant_over_sources_once_perturbed():
    net = perturbed(flow())
    x, t, y, f = rand((S, X), 1), jnp.asarray(0.3), rand((H, W, C), 2), jnp.asarray(F)
    perm = jnp.asarray([3, 1, 0, 2])
    dx, xt, yt = net(x, t, y, f)
    perm_dx, perm_xt, perm_yt = net(x[perm], t, y, f)
...
def test_x_stream_has_no_positional_embedding():
    ...
    dx = net(x, jnp.asarray(0.3), rand((H, W, C), 2), jnp.asarray(F))[0]
...
def test_velocity_is_differentiable_in_x():
    net, t, y = perturbed(flow()), jnp.asarray(0.3), rand((H, W, C), 2)
    f = jnp.asarray(F)
    g = jax.jacobian(lambda z: net(z, t, y, f)[0])(rand((S, X), 1))
...
def test_non_square_window():
    net = flow(width=32)
    dx, _, yt = net(
        rand((S, X), 1), jnp.asarray(0.3), rand((H, 32, C), 2), jnp.asarray(F)
    )
...
def test_y_target_shape_survives_every_patch_stage_count(stages):
    ...
    yt = net(rand((S, X), 1), jnp.asarray(0.3), rand((H, W, C), 2), jnp.asarray(F))[2]
    assert yt.shape == (H, W, C)


def test_rejects_a_window_the_patch_stages_do_not_divide():
