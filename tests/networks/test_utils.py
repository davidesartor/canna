...
from canna.networks import FeedForward, Modulation, SinusoidalEmbed
...
def test_feedforward_out_dim_is_out_dim():
    ff = FeedForward(3, 16, 7, key=key())
    assert ff(rand((3,))).shape == (7,)


def test_feedforward_takes_one_example_and_vmaps():
    ff = FeedForward(3, 16, 7, key=key())
    assert ff(rand((3,))).shape == (7,)
    assert jax.vmap(ff)(rand((5, 3))).shape == (5, 7)
    assert jax.vmap(jax.vmap(ff))(rand((2, 5, 3))).shape == (2, 5, 7)


def test_feedforward_rowwise_independent():
    ff = FeedForward(3, 16, 7, key=key())
    x = rand((4, 3))
    close(jax.vmap(ff)(x), jnp.stack([ff(row) for row in x]))


def test_feedforward_odd_hidden_dim_shape():
    ff = FeedForward(3, 13, 7, key=key())
    assert ff(rand((3,))).shape == (7,)


def test_feedforward_hidden_dim_one():
    ff = FeedForward(3, 1, 7, key=key())
    assert ff(rand((3,))).shape == (7,)


def test_feedforward_unit_dims():
    ff = FeedForward(1, 4, 1, key=key())
    assert ff(rand((1,))).shape == (1,)
...
def test_feedforward_finite_on_large_input():
    ff = FeedForward(3, 16, 7, key=key())
    assert bool(jnp.all(jnp.isfinite(ff(1e4 * rand((3,))))))
...
def test_modulation_broadcasts_over_leading_axes_of_x():
    mod = Modulation(8, key=key())
    c = rand((8,), 2)
    for lead in [(), (5,), (2, 5)]:
        out, gate = mod(rand(lead + (8,), 1), c)
...
def test_modulation_rowwise_independent():
    ...
    out, gate = jax.vmap(mod)(x, c)
    outs = [mod(x[i], c[i]) for i in range(4)]
    close(out, jnp.stack([o for o, _ in outs]))
    close(gate, jnp.stack([g for _, g in outs]))


def test_modulation_conditions_every_token_from_one_vector():
    mod = Modulation(8, key=key())
    out, gate = mod(rand((6, 8), 1), rand((8,), 2))
    assert out.shape == (6, 8)
    assert gate.shape == (8,)


def test_modulation_dim_one():
    mod = Modulation(1, key=key())
    out, gate = mod(rand((4, 1), 1), rand((1,), 2))
    assert out.shape == (4, 1) and gate.shape == (1,)
...
def test_feedforward_at_the_origin_is_the_gated_bias_path():
    ff = FeedForward(3, 16, 7, key=key())
    hidden, gate = jnp.split(ff.linear1.bias, 2)
    reference = ff.linear2.weight @ (hidden * jax.nn.silu(gate)) + ff.linear2.bias
    close(ff(jnp.zeros((3,))), reference, atol=1e-12)


def test_feedforward_is_locally_linear_near_the_origin():
    ff = FeedForward(3, 16, 7, key=key())
    x, base = rand((3,)), ff(jnp.zeros((3,)))
    small = jnp.linalg.norm(ff(1e-5 * x) - base)
    smaller = jnp.linalg.norm(ff(1e-6 * x) - base)
    close(smaller * 10.0, small, atol=1e-9)


def test_feedforward_hidden_dim_is_post_split_width():
    wide = FeedForward(3, 16, 7, key=key())
    narrow = FeedForward(3, 8, 7, key=key())
    assert wide.linear2.weight.shape == (7, 16)
    assert narrow.linear1.weight.shape == (16, 3)


def test_feedforward_gate_half_is_second_half_of_the_projection():
    ff = FeedForward(2, 4, 2, key=key())
    ff = eqx.tree_at(lambda m: m.linear1.weight, ff, jnp.zeros_like(ff.linear1.weight))
    ff = eqx.tree_at(
        lambda m: m.linear1.bias, ff, jnp.concat([jnp.ones((4,)), jnp.zeros((4,))])
    )
    # gate half is exactly zero, so the whole hidden activation is gated off
    close(ff(rand((2,))), ff.linear2.bias, atol=1e-12)
...
def test_modulation_uses_c_once_params_are_perturbed():
    mod = perturbed(Modulation(8, key=key()))
    x = rand((8,), 1)
...
def test_modulation_standardizes_only_the_last_axis():
    ...
    out, _ = mod(x, rand((8,), 2))
    out_scaled, _ = mod(scaled, rand((8,), 2))
    close(out[1:], out_scaled[1:])


def test_modulation_gate_is_one_vector_for_the_whole_token_axis():
    mod = perturbed(Modulation(8, key=key()))
    out, gate = mod(rand((6, 8), 1), rand((8,), 2))
    assert out.shape == (6, 8)
    assert gate.shape == (8,)
...
def test_sinusoidal_scalar_gives_dim_vector():
    emb = SinusoidalEmbed(16, key=key())
    assert emb(jnp.asarray(0.3)).shape == (16,)


def test_sinusoidal_appends_the_dim_axis_under_vmap():
    emb = SinusoidalEmbed(16, key=key())
    assert jax.vmap(emb)(rand((5,))).shape == (5, 16)
    assert jax.vmap(jax.vmap(emb))(rand((2, 5))).shape == (2, 5, 16)


def test_sinusoidal_odd_dim_shape():
    emb = SinusoidalEmbed(15, key=key())
    assert emb(jnp.asarray(0.3)).shape == (15,)


def test_sinusoidal_dim_one_shape():
    emb = SinusoidalEmbed(1, key=key())
...
def test_sinusoidal_elementwise_over_batch():
    emb = SinusoidalEmbed(16, key=key())
    t = rand((7,))
    close(jax.vmap(emb)(t), jnp.stack([emb(ti) for ti in t]))
