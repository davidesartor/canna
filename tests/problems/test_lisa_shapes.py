...
def test_clean_signal_preserves_leading_batch_axis():
    problem = LisaGB(n_sources=2)
    p = jax.vmap(problem.sample_physical)(jr.split(jr.key(0), 4))
    o = problem.clean_signal(p)
    assert o.shape[0] == 4
    assert o.shape[-1] == 3
...
def test_preprocess_time_axis_divisible_by_patch_downsample():
    # preprocess sets nt = F // wdm_freq_bands, and clean_signal rounds its F axis up
    # to a multiple of 2 * wdm_freq_bands * patch_downsample, so nt is a multiple of
    # patch_downsample by construction.
    problem = LisaGB(n_sources=1)
    p = problem.sample_physical(jr.key(48))
    o = problem.clean_signal(p)
    img = problem.preprocess(o)
...
def test_train_sample_shapes_line_up():
    problem = LisaGB(n_sources=3)
    width = problem.prior.chart.flow_dim
...
def test_preprocess_frequency_axis_is_wdm_freq_bands():
    # from_freq_to_wdm returns wdm_freq_bands + 1 frequency bins and preprocess's
    # trailing `[..., 1:]` drops the DC bin, leaving exactly wdm_freq_bands.
    problem = LisaGB(n_sources=1)
    p = problem.sample_physical(jr.key(49))
    o = problem.clean_signal(p)
    img = problem.preprocess(o)
...
def test_clean_signal_broadcasts_over_two_leading_batch_axes():
    ...
    ps = jax.vmap(problem.sample_physical)(jr.split(jr.key(0), 6))
    ps_2d = ps.reshape(2, 3, *ps.shape[1:])
    h_2d = problem.clean_signal(ps_2d)
    h_flat = problem.clean_signal(ps)
...
def test_preprocess_broadcasts_over_two_leading_batch_axes():
    ...
    img_2d = problem.preprocess(os_2d)
    img_flat = problem.preprocess(os_)
...
def test_snr_broadcasts_over_two_leading_batch_axes():
    ...
    ps = jax.vmap(problem.sample_physical)(jr.split(jr.key(0), 6))
    ps_2d = ps.reshape(2, 3, *ps.shape[1:])
    snr_2d = problem.snr(ps_2d)
    snr_flat = problem.snr(ps)
