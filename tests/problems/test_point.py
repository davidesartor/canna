...
def test_noisy_point_default_dim_is_two():
    problem = NoisyPoint()
    assert problem.chart.physical_dim == 2
    assert problem.chart.flow_dim == 2
    assert problem.geometry.dim == 2


def test_noisy_point_chart_and_geometry_types():
    problem = NoisyPoint()
    assert isinstance(problem.chart, Affine)
    assert isinstance(problem.geometry, Euclidean)
...
def test_sample_observation_clean_ignores_noise_and_key():
    problem = NoisyPoint(noise_std=2.0)
    p = jnp.array([1.5, -2.5])
...
def test_preprocess_matches_chart_forward():
    """preprocess(o) == chart.forward(o), whitening the
    observation with the same Affine chart used for the physical->point map."""
    problem = NoisyPoint()
    o = jnp.array([3.3, -7.1])
    assert jnp.allclose(problem.preprocess(o), problem.chart.forward(o), atol=1e-6)
...
def test_sample_point_equals_chart_forward_of_sample_physical_same_key():
    ...
    assert jnp.allclose(
        ...
        problem.chart.forward(problem.sample_physical(key)),
        atol=1e-6,
    )
...
def test_prior_covariance_is_spd():
    cov = NoisyPoint(dim=4).prior.cov
...
def test_prior_covariance_cholesky_succeeds():
    cov = NoisyPoint(dim=5).prior.cov
...
def test_prior_covariance_is_full_not_diagonal():
    cov = NoisyPoint(dim=3).prior.cov
...
def test_chart_whitening_roundtrip_recovers_physical_point():
    ...
    assert jnp.allclose(problem.chart.backward(problem.chart.forward(p)), p, atol=1e-4)


def test_prior_mean_is_zero():
    assert jnp.allclose(NoisyPoint(dim=3).prior.mean, 0.0, atol=1e-8)


def test_same_seed_is_deterministic():
    a, b = NoisyPoint(dim=3, seed=7), NoisyPoint(dim=3, seed=7)
    assert jnp.allclose(a.prior.cov, b.prior.cov, atol=0.0)


def test_different_seed_gives_a_different_covariance():
    a, b = NoisyPoint(dim=3, seed=0), NoisyPoint(dim=3, seed=1)
    assert not jnp.allclose(a.prior.cov, b.prior.cov, atol=1e-3)


def test_dim_sets_the_covariance_size():
    assert NoisyPoint(dim=6).prior.cov.shape == (6, 6)
...
def test_empirical_correlation_matches_prior_covariance():
    ...
    assert jnp.allclose(empirical, problem.prior.cov, atol=0.1)
