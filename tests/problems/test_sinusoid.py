...
from canna.problems import NoisySinusoid
...
def test_sample_point_matches_chart_forward_of_physical(problem, key):
    ...
    x_via_chart = problem.chart.forward(p)
    assert jnp.allclose(x_direct, x_via_chart, atol=1e-4)


def test_prior_matches_sample_physical(problem, key):
    """sample_physical is literally `return self.prior(key)`."""
    assert jnp.allclose(problem.sample_physical(key), problem.prior(key), atol=1e-4)
...
def test_sample_observation_clean_matches_clean_signal(key):
    problem = NoisySinusoid(n_sources=2, noise_level=1.0)
    p = problem.sample_physical(key)
    o = problem.sample_observation(key, p, clean=True)
    assert jnp.allclose(o, problem.clean_signal(p), atol=1e-5)
...
def test_train_sample_x_target_on_manifold(problem, key):
    ...
    (confirmed from priors.Product's amp=,freq=,phase= kwarg order and charts.Product's
    per-block concatenation); the last 2 dims are the (cos,sin) pair, unit-norm."""
    sample = problem.train_sample(key)
...
def test_geodesic_t1_endpoint_can_differ_from_raw_x_target(problem):
    ...
    train_sample's x_target is the raw, un-reordered chart.forward(p) (base.py: x1 =
    self.chart.forward(p)). So geometry.geodesic(1, x0, x_target) is Set.assign(x0,
    ...
    for seed in range(20):
        key = jr.key(seed)
        # mirrors base.train_sample's split; unpacked in full so a change here breaks loudly
        _key_p, _key_o, _key_o_target, key_x0, _key_t = jr.split(key, 5)
        x0 = problem.sample_point(key_x0)
        sample = problem.train_sample(key)
        # x0 is the one train_sample actually drew, else the rest is vacuous
        assert jnp.allclose(
            problem.geometry.geodesic(sample.t, x0, sample.x_target),
            sample.xt,
            atol=1e-4,
        )
        g1 = problem.geometry.geodesic(jnp.array(1.0), x0, sample.x_target)
