...
def test_product_geometry_dim_with_curved_blocks():
    g = ProductGeometry(Toroidal(dim=4), Spherical(dim=3))
    assert g.dim == 7


def test_product_geometry_over_curved_blocks_geodesic_endpoints():
    g = ProductGeometry(Toroidal(dim=2), Spherical(dim=3))
    x0 = jnp.concatenate(
...
def test_set_geometry_over_toroidal_dim_ignores_set_size():
    # dim reflects one element's flat width, S is not folded in
    geo = SetGeometry(Toroidal(dim=4))
    assert geo.dim == 4


def test_set_geometry_over_toroidal_assign_identity_on_matching_sets():
    geo = SetGeometry(Toroidal(dim=2))
